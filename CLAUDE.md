# Claude Code Project Instructions — shlex-kotlin

## Project Overview

This is **shlex-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`shlex`](https://crates.io/crates/shlex).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/shlex/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/shlex>
